...
**Step 1.5: 代码一致性校验（新增检查点）**

**触发条件：**
```javascript
const shouldTrigger = (
  node.type === 'code' &&
  node.error?.match(/ReferenceError|not defined|变量未定义/) &&
  !node.code?.includes(errorVariable)
);
```
...
【现象】报错说变量 "{{errorVariable}}" 未定义
【检查】当前代码中确实没有这个变量

可能原因（按概率排序）：

□ A. 【最常见】代码已修改但未发布
   └─ Dify 执行的是 Publish 版本，Draft 已修改但未发布
   └─ 解决：点击"发布"后重试

□ B. 【次常见】上游节点未输出该变量
   └─ 前一个节点改了输出变量名，或执行失败没输出
   └─ 检查：查看上游节点的"上次运行"输出
...
**分支处理：**
- **A/B/C/E/F**：按 `references/code-consistency-troubleshooting.md` 执行详细排查流程
- **D**：进入版本对比流程（详见 references 第 4 节）
- **所有分支排查后仍无果**：回到 Step 2 构建假设树，继续苏格拉底式排查
